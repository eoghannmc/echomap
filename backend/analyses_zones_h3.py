...
def _geom_to_wgs84_fc(geom):
    ...
    for g in geoms:
        if g.is_empty:
            continue
        coords = [(to_wgs84.transform(x, y)) for x, y in g.exterior.coords]
        feats.append({"type": "Feature",
                      "geometry": {"type": "Polygon", "coordinates": [[(lon, lat) for (lon, lat) in coords]]},
                      "properties": {}})
    return {"type": "FeatureCollection", "features": feats}

from pyogrio import errors as pyogrio_errors  # already imported above
# ... keep previous helpers ...

class ZonesAnalysisH3:
    def __init__(self) -> None:
        pass  # no heavy I/O at import time

    @staticmethod
    @lru_cache(maxsize=128)
    def _cached_ring_key(center_cell: str, res: int, k: int, band_index: int, codes_key: str, tol_mm: int) -> str:
    ...
    def run(
        ...
        if gdf.empty:
            return {
                ...
                "mask": _geom_to_wgs84_fc(clip_geom),
                "summary": {
                    "count": 0,
                    "clip_mode": clip_mode,
                    "ring_selected": bi,
        ...
        if len(clipped) > max(0, int(max_features)):
            clipped = clipped.iloc[: int(max_features)].copy()
        if simplify_tolerance_m and not clipped.geom_type.isin(["Point", "MultiPoint"]).all():
            clipped["geometry"] = clipped.geometry.simplify(float(simplify_tolerance_m), preserve_topology=True)

        out = clipped.to_crs(4326)
        feats = []
        for _, r in out.iterrows():
            geom = r.geometry
            if geom is None or geom.isNil if False else False:  # guard path; geom.is_empty is enough
                continue
            if geom.is_empty:
                continue
            feats.append({
